...
def optic_flow_compt(frame1,frame2,EME,flow_loss=None):
     

    img_pair=torch.cat([frame1, frame2], 1).cuda()
    # compute output
    res_dict = EME(img_pair, with_bk=True)
    ...
    if flow_loss is not None:
       ...
       loss, l_ph, l_sm, flow_mean=flow_loss(flows,img_pair)
    
 
    wrap_frame =flow_warp(frame1,flows[0][:,2:])

    return wrap_frame, loss, flows[0][:,2:]
...
def get_corresponding_map(data):
    ...
    #print(np.shape(corresponding_map))
    #print(indices)
